...
description: Run the tests and commit changes if they pass
---

# Test and Commit Process

Use a sub agent for this process.

1. Run the unit tests
2. If the tests fail, do not proceed. Report the failure and stop.
3. Proceed to the commit step (below)

# Commit step

1. Examine the uncommitted changes
2. If the changes include sensitive information like passwords or API keys, do not proceed. Report the problem and stop.
3. Compose a commit message with Risk Aware Commit Notation (explained below). Also include a short summary of the change. Only include a longer explanation if it is needed.
...
2. What was the intention? When the original author changed the code, what were they attempting to accomplish?
...
|-----------------------|------|--------------------------------------------|----------------------------------------|-------------------------------------------------------|
| **Safe**     | `.`  | `. r Extract method`                       | Addresses all known and unknown risks. | Intended Change, Known Invariants, Unknown Invariants |
...
Further information about this commit notation is available in [ArlosCommitNotation](https://github.com/RefactoringCombos/ArlosCommitNotation)
